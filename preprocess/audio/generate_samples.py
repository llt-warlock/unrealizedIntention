...
unsuccessful_intention = [(2, 386160, 388520, 'INTS_start'), (3, 202950, 204750, 'INTS_start'), (3, 208510, 211840, 'INTS_start'),
                          (3, 371673, 372980, 'INTS_continue'), (4, 42000, 46320, 'INTS_continue'), (4, 111633, 115986, 'INTS_start'),
                          (4, 127933, 129653, 'INTS_start'), (4, 175846, 178086, 'INTS_continue'), (4, 250906, 254353, 'INTS_continue'),
                          (4, 263833, 267353, 'INTS_continue'), (4, 284286, 286060, 'INTS_continue'), (4, 295220, 298173, 'INTS_continue'),
                          (4, 310566, 311986, 'INTS_continue'), (4, 428740, 429906, 'INTS_start'), (4, 454213, 456400, 'INTS_start'),
                          (4, 461800, 464920, 'INTS_start'), (7, 260780, 261773, 'INTS_maybe'), (7, 573726, 576053, 'INTS_start'),
                          (10, 298766, 302260, 'INTS_continue'), (10, 327113, 327926, 'INTS_start'), (11, 200620, 202100, 'INTS_start'),
                          (11, 265993, 268886, 'INTS_continue'), (11, 453580, 455440, 'INTS_continue'), (11, 504746, 506253, 'INTS_start'),
                          (11, 569646, 572020, 'INTS_start'), (11, 584906, 586813, 'INTS_continue'), (17, 126940, 128820, 'INTS_start'),
                          (17, 419506, 421513, 'INTS_maybe'), (17, 447546, 451213, 'INTS_continue'), (22, 270433, 272220, 'INTS_continue'),
                          (22, 320566, 321906, 'INTS_continue'), (22, 356313, 358780, 'INTS_start'), (22, 532260, 534773, 'INTS_continue'),
                          (22, 555600, 556740, 'INTS_start'), (23, 371340, 374440, 'INTS_start'), (27, 169253, 170973, 'INTS_continue'),
                          (27, 260046, 262333, 'INTS_maybe'), (34, 423106, 424286, 'INTS_start'), (34, 425030, 426220, 'INTS_start'),
                          (34, 431330, 432480, 'INTS_start'), (34, 439420, 440633, 'INTS_start'), (34, 477426, 479106, 'INTS_start'),
                          (34, 513746, 515126, 'INTS_continue'), (35, 378093, 379920, 'INTS_continue'), (5, 412920, 414420, 'INTS_maybe')]
...
def generate_negative_sample(train_intention_time_window, test_intention_time_window, intention_label, duration, ratio,
                             size=9900,
...
def generate_negative_unsuccessful_sample(positive_sample, successful_ground_truth, ground_truth, duration, ratio, fs=100):
    negative_sample = []
    ...
    while len(negative_sample) < number_of_negative_sample:
        ...
        if left_point >= 0:
            if not ground_truth[(left_point + 3600) * fs:(random_point + 3600) * fs].__contains__(1) and not successful_ground_truth[(left_point + 3600) * fs:(random_point + 3600) * fs].__contains__(1):
        ...
        if right_point <= 600:
            if not ground_truth[(random_point + 3600) * fs: (right_point + 3600) * fs].__contains__(1) and not successful_ground_truth[(random_point + 3600) * fs: (right_point + 3600) * fs].__contains__(1):
...
if __name__ == '__main__':
    ...
    for sample_index in range(0, 100):
        ...
        for k in range(0, len(pid_list)):
            ...
            neg_example_train_time_list, neg_example_test_time_list = generate_negative_sample(
                pos_example_train_time_list,
            ...
            with open('./po_ne_csv/' + str(sample_index) + "_" + str(pid_list[k]) + '.csv', "w") as f:
            ...
            with open('./test_data_po_ne_csv/' + str(sample_index) + "_" + str(pid_list[k]) + '.csv', "w") as ff:
            ...
            unsuccessful_negative_sample = generate_negative_unsuccessful_sample(unsuccessful_positive_sample, label_pos,
                                                                                 unsuccessful_label, 4, 20)
            ...
            if start_pid.__contains__(pid_list[k]):
                with open('./unsuccessful_intention_sample/' + str(sample_index) + "_" + str(pid_list[k]) + '.csv',
                          "w") as fff:
                ...
                if sample_index == 0:
                    outfile_unsuccessful_intention_label = open(
                        './unsuccessful_intention_label/' + str(pid_list[k]) + '.csv', "w", newline='')
